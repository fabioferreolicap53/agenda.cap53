...
const TeamManagement: React.FC = () => {
    ...
    return (
        <div className="flex flex-col gap-4 max-w-[1500px] mx-auto w-full p-4">
                        ...
                        return filteredUsers.map((u, idx) => {
                            ...
                            return (
                                <div key={userData.id || idx} className={`bg-white rounded-3xl transition-all duration-300 group relative flex flex-col ${isMe && editingSector ? 'z-[100]' : 'overflow-hidden'} ${isMe
                                    ...
                                    <div className="relative">
                                        <div className="h-24 relative overflow-hidden rounded-t-3xl">
                                            ...
                                            <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {unreadCounts[userData.id] > 0 && (
                                                            <button 
                                                                ...
                                                                className="bg-red-500 text-white text-[10px] font-bold h-7 px-2 flex items-center justify-center rounded-lg border-2 border-white shadow-lg animate-pulse"
                                                            >
                                                                <span className="material-symbols-outlined text-[14px] mr-1">chat</span>
                                                                {unreadCounts[userData.id]}
                                                            </button>
                                                        )}
                                                        <button
                                                            onClick={() => toggleUserSelection(userData.id)}
                                                            className={`w-7 h-7 rounded-lg border-2 transition-all duration-300 flex items-center justify-center backdrop-blur-md ${
                                                ...
                                                {isMe && (
                                                    <div className="bg-primary/90 backdrop-blur-md text-white text-[9px] font-black px-2 py-1 rounded-lg uppercase tracking-[0.15em] shadow-lg shadow-primary/20 border border-white/20">
                                                        Você
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className="absolute -bottom-10 left-6 z-20 transition-transform duration-500 group-hover:scale-105">
                                            <div className="relative flex flex-col items-center">
                                                <div
                                                    className="size-20 rounded-2xl bg-contain bg-center bg-no-repeat bg-slate-100 border-[5px] border-white shadow-xl shadow-black/5 ring-1 ring-black/5"
                                                    style={{ backgroundImage: `url(${getAvatarUrl(userData)})` }}
                                                ></div>
                                                <div className={`absolute -bottom-1 -right-1 size-5 border-[3px] border-white rounded-full ${getStatusColor(userData.status)} shadow-lg ring-1 ring-black/5 flex items-center justify-center`}>
                                                </div>
                                                
                                                {/* User Status Badge */}
                                                <div className="absolute top-0 -right-2 transform translate-x-full mt-1">
                                                    <span className={`text-[8px] font-black uppercase tracking-[0.1em] px-2 py-0.5 rounded-md border shadow-sm whitespace-nowrap ${
                                    ...
                                    <div className="pt-12 p-6 flex flex-col gap-5 flex-1">
                                        {/* Identity Section */}
                                                ...
                                                {isMe && editingDetails ? (
                                                    <div className="relative flex-1 group/input">
                                                        <input
                                                            ...
                                                            className={`text-lg font-black text-text-main border-b-2 outline-none bg-transparent w-full pb-1 transition-all ${focusedField === 'name' ? 'border-primary' : 'border-gray-100'}`}
                                                            placeholder="Seu Nome"
                                                        />
                                                        <div className={`absolute bottom-0 left-0 h-0.5 bg-primary transition-all duration-300 ${focusedField === 'name' ? 'w-full' : 'w-0'}`}></div>
                                                    </div>
                                                ) : (
                                                    <h3 className="text-lg font-black text-text-main group-hover:text-primary transition-colors truncate tracking-tight">
                                                ...
                                                {isMe ? (
                                                    <div className="relative">
                                                        <button
                                                            ...
                                                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border transition-all text-[10px] font-bold uppercase tracking-wide group/sectbtn ${
                                                                editingSector 
                                                        ...
                                                        >
                                                            ...
                                                            <span className={`material-symbols-outlined text-[14px] transition-transform duration-300 ${editingSector ? 'rotate-180' : ''}`}>expand_more</span>
                                                        </button>
                                        ...
                                        <div className="flex flex-col gap-3 pt-2">
                                            <div className="flex items-center gap-4 group/contact">
                                                <div className="size-9 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 transition-colors group-hover/contact:bg-primary/5 group-hover/contact:text-primary">
                                                    <span className="material-symbols-outlined text-[18px]">mail</span>
                                                </div>
                                                <div className="flex flex-col flex-1 min-w-0">
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">E-mail Corporativo</span>
                                                    <span className="text-xs font-bold text-slate-600 truncate">{userData.email}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-4 group/contact">
                                                ...
                                                <div className="flex flex-col flex-1 min-w-0">
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Telefone / WhatsApp</span>
                                                    <div className="flex items-center justify-between gap-2">
                                                        {isMe && editingDetails ? (
                                                            <input
                                                                ...
                                                                onBlur={handleFieldBlur}
                                                                className={`text-xs font-bold text-text-main border-b outline-none bg-transparent w-full py-0.5 transition-all ${focusedField === 'phone' ? 'border-primary' : 'border-gray-100'}`}
                                                            />
                                                        ) : (
                                                            <span className="text-xs font-bold text-slate-600">{userData.phone || 'Não informado'}</span>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="mt-2 flex flex-col gap-3 bg-slate-50/50 rounded-2xl p-4 border border-slate-100/50 group-hover:bg-white group-hover:border-primary/10 transition-all duration-500">
                                            <div className="flex justify-between items-center">
                                                <div className="flex items-center gap-2">
                                                    <span className="material-symbols-outlined text-[16px] text-primary">info</span>
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Informações Adicionais</span>
                                                </div>
                                                {isMe && (
                                                    <button
                                                    ...
                                                    >
                                                        <span className="material-symbols-outlined text-[14px]">{editingBio ? 'close' : 'edit'}</span>
                                                    </button>
                                                )}
                                            </div>
                                            ...
                                            {isMe && editingBio ? (
                                                <div className="flex flex-col gap-3 animate-in fade-in duration-300">
                                                    <textarea
                                                        ...
                                                        onBlur={handleFieldBlur}
                                                        className={`text-[11px] font-medium text-slate-600 leading-relaxed bg-white border rounded-xl p-3 outline-none min-h-[100px] transition-all resize-none ${focusedField === 'bio' ? 'border-primary ring-4 ring-primary/5' : 'border-gray-100'}`}
                                                        placeholder="Escreva um pouco sobre suas responsabilidades..."
                                                    />
                                                    <button
                                                        onClick={handleSaveBio}
                                                        disabled={isSaving}
                                                        className="w-full py-2 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 hover:bg-primary-hover transition-all flex items-center justify-center gap-2"
                                                    >
                                                        {isSaving ? (
                                                            <>
                                        ...
                                        {!isMe && (
                                            <div className="mt-auto pt-2">
                                                <button 
                                                    onClick={() => navigate(`/chat?userId=${userData.id}`)}
                                                    className="w-full flex items-center justify-center gap-2 py-2.5 bg-primary/5 text-primary hover:bg-primary hover:text-white rounded-xl transition-all duration-300 group/btn"
                                                >
                                                    <span className="material-symbols-outlined text-[18px]">chat</span>
                                                    <span className="text-[10px] font-black uppercase tracking-wider">Conversar</span>
                                                </button>
                                            </div>
                                        )}
                                    </div>
            ...
            {selectedUsers.length > 0 && (
                <div className="fixed bottom-8 right-8 z-[100] animate-in zoom-in slide-in-from-bottom-10 duration-500">
                    <button
                        onClick={handleCreateEventWithSelected}
                        className="group flex items-center gap-3 bg-primary text-white px-6 py-4 rounded-2xl shadow-2xl shadow-primary/30 hover:bg-primary-hover hover:scale-105 active:scale-95 transition-all"
                    >
                        <div className="flex -space-x-2">
                            {selectedUsers.slice(0, 3).map((id, i) => {
                                const user = users.find(u => u.id === id);
                                return (
                                    <div key={id} className="size-6 rounded-lg border-2 border-primary bg-white overflow-hidden shadow-lg">
                                        <img src={getAvatarUrl(user)} alt="" className="w-full h-full object-cover" />
                                    </div>
                                );
                            })}
                            {selectedUsers.length > 3 && (
                                <div className="size-6 rounded-lg border-2 border-primary bg-white flex items-center justify-center text-[10px] font-black text-primary">
                                    +{selectedUsers.length - 3}
                                </div>
                            )}
                        </div>
                        <div className="flex flex-col items-start">
                            <span className="text-[10px] font-black uppercase tracking-widest leading-none">Agendar com Equipe</span>
                            <span className="text-[11px] font-bold opacity-80">{selectedUsers.length} {selectedUsers.length === 1 ? 'membro selecionado' : 'membros selecionados'}</span>
                        </div>
