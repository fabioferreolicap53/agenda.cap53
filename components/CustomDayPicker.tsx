...
const CustomDayPicker: React.FC<CustomDayPickerProps> = ({ value, onChange, className = '', eventsByDate = {} }) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent | TouchEvent) => {
            ...
            if (!clickedInButton && !clickedInPopup) {
                setIsOpen(false);
            }
        };
        ...
        for (let i = firstDayOfMonth - 1; i >= 0; i--) {
            ...
            days.push(
                <button
                    ...
                    className="h-10 w-10 lg:h-8 lg:w-8 flex items-center justify-center text-[11px] lg:text-[10px] text-slate-300 hover:bg-slate-50 rounded-lg transition-colors"
                >
                    {prevMonthDays - i}
                </button>
            );
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            days.push(
                <button
                    key={`curr-${i}`}
                    onClick={() => handleSelectDate(d)}
                    className={`h-9 w-9 lg:h-8 lg:w-8 flex flex-col items-center justify-center text-[12px] lg:text-[10px] font-bold rounded-xl lg:rounded-lg transition-all duration-300 relative active:scale-90 ${
                        isSelected 
                ...
                >
                    ...
                    {hasEvents && !isSelected && (
                        <span className={`absolute bottom-1.5 lg:bottom-1 size-1 rounded-full transition-transform duration-300 ${
                            isToday ? 'bg-primary' : isWeekend ? 'bg-orange-400/60' : 'bg-slate-400/60'
                        }`}></span>
                    )}
                    {isSelected && (
                        <span className="absolute inset-0 bg-white/10 rounded-xl lg:rounded-lg animate-pulse"></span>
                    )}
                </button>
            );
        }
        ...
        <div ref={containerRef} className={`relative ${className}`}>
            ...
            {isOpen && createPortal(
                <>
                    ...
                    <div 
                        className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] z-[190] lg:hidden animate-in fade-in duration-300"
                    ...
                    >
                        <div className="flex items-center justify-between mb-4 lg:mb-4">
                            <button 
                                ...
                                className="size-9 lg:size-9 flex items-center justify-center hover:bg-slate-50 rounded-full transition-all text-slate-300 hover:text-primary active:scale-90"
                            >
                                <span className="material-symbols-outlined text-xl lg:text-xl">chevron_left</span>
                            </button>
                            <div className="text-[12px] lg:text-[11px] font-black uppercase tracking-[0.1em] text-slate-800">
                            ...
                            <button 
                                ...
                                className="size-9 lg:size-9 flex items-center justify-center hover:bg-slate-50 rounded-full transition-all text-slate-300 hover:text-primary active:scale-90"
                            >
                                <span className="material-symbols-outlined text-xl lg:text-xl">chevron_right</span>
                            </button>
                        </div>

                        <div className="grid grid-cols-7 gap-1 mb-2 lg:mb-2">
                            {DAYS_SHORT.map((day, index) => {
                                const isWeekend = index === 0 || index === 6;
                                return (
                                    <div 
                                        key={`header-${index}`} 
                                        className={`h-7 lg:h-7 flex items-center justify-center text-[9px] lg:text-[9px] font-black uppercase tracking-widest ${
                        ...
                        <div className="mt-5 lg:mt-5 pt-4 lg:pt-4 border-t border-slate-50 flex justify-between gap-3 lg:gap-3">
                            <button 
                                onClick={() => handleSelectDate(new Date())}
                                className="flex-1 py-2.5 lg:py-2.5 text-[10px] lg:text-[10px] font-black uppercase tracking-[0.15em] text-slate-800 bg-slate-50 hover:bg-slate-100 rounded-[14px] lg:rounded-xl transition-all active:scale-95 shadow-sm"
                            >
                                HOJE
                            </button>
                            <button 
                                onClick={() => setIsOpen(false)}
                                className="flex-1 py-2.5 lg:py-2.5 text-[10px] lg:text-[10px] font-black uppercase tracking-[0.15em] text-slate-300 bg-slate-50/50 hover:bg-slate-50 rounded-[14px] lg:rounded-xl transition-all active:scale-95 shadow-sm"
                            >
                                FECHAR
                            </button>
                        </div>
