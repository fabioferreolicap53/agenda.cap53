...
async function run() {
    try {
        console.log('Starting debug update script...');
        
        // 1. Authenticate first
        const email = 'dca@cap53.com';
        ...
        // 2. Fetch an item
        console.log('Fetching an item to test...');
        const res = await fetch(`${PB_URL}/api/collections/agenda_cap53_itens_servico/records?limit=1`, {
        ...
        if (data.items && data.items.length > 0) {
            ...
            console.log(`Testing with item: ${item.name} (${itemId}) - Current: ${item.is_available}`);
            ...
            // 5. Revert back (to keep data clean)
            ...
            console.log('No items found in agenda_cap53_itens_servico');
        }
